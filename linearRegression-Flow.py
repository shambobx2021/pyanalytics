...
#x = np.array([5,15,25,35,45,55]) #what if x is not a 2-d array
x  #Ind. Variable
x.shape
x.ndim

y = np.array([5,20,14,32,22, 38])  #1 dim
y #Dep. Variable #(y can 2 dim also : y.reshape((-1,1)))
y.shape
y.ndim
#S3 - Model
...
#%% MUltiple Linear Regression - 2 inde. variables
import numpy as np
from sklearn.linear_model import LinearRegression
...
#%% MUltiple Linear Regression - 3 inde. variables
import numpy as np
from sklearn.linear_model import LinearRegression

x = [[0,1,3], [5,1,4], [15,2,2], [25,2,6], [35,11,6], [45,15,9], [55,34,12], [60,35,14]]
x
type(x)
...
y
x, y = np.array(x), np.array(y)
x #2 dim ; MLR - 3 variable, 2 dim(LxB) with 3 columns
y #1 dim
x.shape, y.shape
x.ndim
y.ndim
